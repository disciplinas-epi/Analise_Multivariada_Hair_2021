...
testestesakcbahbvcabjhvbasjkv


Source: [Github clemonster](https://github.com/clemonster/decathlon-pca)   
**Obs.**: Acrescentei alguns gráficos e formatação ao script original.  

Aulas do autor do pacote FactoMiner [François Husson](http://factominer.free.fr/more/EDA_PCA.pdf)   
Link de [tutoriais e aulas](https://husson.github.io/tutorials.html)  
...
# Dá pra arrumar e retirar algumas informações
fviz_pca(res.pca)

```

Visualização comum e bonita do factoextra.
```{r}
summary(res.pca)
...
fviz_pca_ind(res.pca, col.ind = "blue", habillage = 13,
             addEllipses = F, repel = FALSE)+
   theme_minimal()

fviz_pca_ind(res.pca, axes = c(1, 3), col.ind = "blue", habillage = 13,
             addEllipses = F, repel = FALSE)+
   theme_minimal()
...
**7.  We can further investigate which sports contribute most to the first 4 dimensions with the following function.**
...
fviz_ellipses(res.pca, habillage = 13, addEllipses = T)
...
### Summary: general interpretation
